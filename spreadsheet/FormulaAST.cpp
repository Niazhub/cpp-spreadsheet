...
namespace ASTImpl {
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const SheetInterface& args) const override {
                double operand_value = operand_->Evaluate(args);
        ...
        public:
            ...
            double Evaluate(const SheetInterface& args) const override {
                if (args.GetCell({ cell_->row, cell_->col }) == nullptr) {
                    return 0.0;
                }
                if (std::holds_alternative<double>(args.GetCell({ cell_->row, cell_->col })->GetValue())) {
                    return std::get<double>(args.GetCell({ cell_->row, cell_->col })->GetValue());
                }
                else if(std::holds_alternative<std::string>(args.GetCell({ cell_->row, cell_->col })->GetValue())){
                    throw FormulaError(FormulaError::Category::Value);
                }
                else if (std::holds_alternative<FormulaError>(args.GetCell({ cell_->row, cell_->col })->GetValue())) {
                    throw std::get<FormulaError>(args.GetCell({ cell_->row, cell_->col })->GetValue());
                }
                return 0.0;
            }
